...
public class ControladorCompanyia implements ActionListener {
	 ...
	 * seleccionar una de les companyies guardades en el vector de companyies del
	 * ControladorPrincipal. - Per seleccionar les companyies, l'usuari ha de
	 * disposar en el JPane d'un botó amb el codi de cadascuna de les companyies,
	 * amb el missatge "Selecciona una companyia", el títol "Seleccionar companyia"
	 * i la icona de qüestió. - Si s'ha seleccionat una companyia, aquesta passarà a
	 * ser la companyia actual del ControladorPrincipal.
	 * 
	 * NOTA: Si abans de seleccionar una companyia tanquem la finestra, el mètode de
	...
	private Boolean validarCompanyia() {
		if (formCompanyia.gettNom() != null && "".equals(formCompanyia.gettNom().getText())) {
			...
			JOptionPane.showMessageDialog(null, "ATENCIÓ!!!", "S'ha d'introduir el nom", JOptionPane.WARNING_MESSAGE,
					null);
			return false;
		}
	}
	 ...
	 * formulari i del llistat, llavors en aquest mètode actionPerformed heu de
	 * controlar si l'usuari ha premut algun botó de qualsevol de les finestres
	 * esmentades.
	 * 
	 * En el cas del formulari i del llistat, com provenen del menú companyia
	 * (s'activen des del menú companyia), heu de verificar primer que els objectes
	 * FormCompanyia o LlistatCompanyia no són nulls, per tal de saber si podeu
	 * comparar-los amb algun botó d'aquestes finestres.
	 * 
	 * Accions per al menú:
	 * 
	 ...
	 * menuCompanyia. També, heu d'actualitzar l'atribut opcioSeleccionada amb
	 ...
	 * Si l'opció seleccionada en el menú companyia és 1 (alta), llavors: Es validen
	 * les dades mitjançant el mètode validarCompanyia(): Si no són correctes,
	 * validarCompanyia() mostrarà l'avís corresponent (penseu que no heu de fer
	 * res, ja que és el mètode el que mostra l'avis directament) Si són correctes:
	 * - Es crea un nou objecte Companyia amb les dades del formulari - S'afegeix la
	 * companyia creada al vector companyies del ControladorPrincipal(recordeu
	 * actualitzar posicioCompanyes un cop afegida) - S'assigna al camp de text
	 * corresponent al codi, el codi de la nova companyia. - S'assigna aquesta
	 * companyia, com a companyiaActual (del ControladorPrincipal) i es canvia
	 * l'atribut opcioSeleccionada a 2 per seleccionar la companyia actual. Si
	 * l'opció seleccionada en el menú companyia és 3 (modificar), llavors: Es
	 * validen les dades mitjançant el mètode validarCompanyia(): Si no són
	 * correctes, validarCompanyia() mostrarà un missatge (No heu de fer res, ja ho
	 * fa validarCompanyia()) Si són correctes: - Es modifica l'objecte companyia
	 * amb les dades introduides mitjançant el formulari (penseu que en aquests
	 * moments, la companyia és la companyia actual)
	 * 
	 * ---- SORTIR ---- Si el botó premut per l'usuari és el botó de sortir del
	 * formulari, llavors: Heu de tornar al menú companyia i amagar el formulari.
	 * 
	...
	private void seleccionarOpcio(int opcio) {
		...
		case 5: // Desar contingut
			 ...
			 * l'usuari "Com vols desar-ho?". La icona sera un interrogant. - Un cop
			 * escollit el mètode, es desa el contingut de companyia cridant al mètode
			 * desarContingut del gestor de persistència i es torna a mostrar el menú de
			 * companyia. - Heu de capturar les excepcions que es puguin produir en el
			 * mètode. Un cop capturada heu de mostrar una finestra emergent de tipus
			 * showMessageDialog de la classe JPane, que ens mostri el missatge d'error
			 * produït amb la icona de error, títol "ERROR" i un cop acceptat torni a
			 * mostrar el menú de companyia.
			 * 
		...
		case 6: // Carregar contingut
			 ...
			 * (XML i JDBC. Recordeu que a ControladorPrincipal hi ha un atribut amb els
			 * mètodes de càrrega).El títol de la finestra serà "Carregar Contingut" i li
			 * demanarem a l'usuari "D'on vols carregar el contingut?". La icona sera un
			 * interrogant. - Un cop l'usuari ha seleccionat el mètode de càrrega: -
			 * S'actualitza el contingut de Companyia mitjançant el mètode carregarContingut
			 * del gestor de persistència. - Es mostra un missatge confirmant que el
			 * contingut s'ha modificat(JOptionPane.showMessageDialog) amb títol "Contingut"
			 * i cap icona. - Es torna a mostrar el menú de companyia. - Heu de capturar les
			 * excepcions que es puguin produir en el mètode. Un cop capturada heu de
			 * mostrar una finestra emergent de tipus showMessageDialog de la classe JPane,
			 * que ens mostri el missatge d'error produït amb la icona de error, títol
			 * "ERROR" i un cop acceptat torni a mostrar el menú d'adminstrador.
			 * 
